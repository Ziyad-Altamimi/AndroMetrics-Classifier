...
|-- generate_runner.sh         # Auto-generates run.sh / run.bat
...
### Step 1: Generate runner scripts

```bash
bash generate_runner.sh
```

Scans `samples/` for APKs and creates `run.sh` / `run.bat` with one Java command per APK. Also appends a command to auto-generate `list_of_csv_files.txt`.
...
```bash
python SEMetrics.py -f list_of_csv_files.txt -csv final_dataset.csv
```

Reads all per-APK CSVs, computes statistical aggregates, writes `final_dataset.csv`, and runs classification automatically. Bonus analysis (ROC curves, CM heatmaps, 10-fold CV) also runs automatically during this step.
